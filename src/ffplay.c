...
typedef struct VideoState {
	...
	//by liqingyu 2013.04.08 start
    int is_file_end_reached;
	int is_video_end_reached;
	int is_audio_end_reached;
	int is_subtitle_end_reached;
	int audio_volume;
    //by end
} VideoState;
...
//static int64_t cursor_last_shown; //by liqingyu 2013.04.12 remove warnning
//static int cursor_hidden = 0; //by liqingyu 2013.04.12 remove warnning
#if CONFIG_AVFILTER
...
#define FF_COMPLETE_EVENT (SDL_USEREVENT + 3)
//by end
...
int is_SDL_init = 0;
HWND hwndSDL;
HWND FFQLAY_hwndParent = NULL;
int FFQLAY_parentWidth;
int FFQLAY_parentHeight;
SDL_mutex *FFQLAY_video_state_mutex = NULL;
#define FFPLAYDEBUG 1
//by end
    ...
    for (;;) {
        ...
        if (pkt1) {
            ...
            *pkt = pkt1->pkt;
            ...
            if( cur_stream->is_file_end_reached == 0 )
	            SDL_CondWait(q->cond, q->mutex);
			else
			{
				ret = -1;
...
static void video_audio_display(VideoState *s)
{
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        //by liqingyu 2013.04.12 start 
        if( FFQLAY_hwndParent != NULL && s->video_st != NULL )
	    {
		    static RECT lastPos;
		    RECT r;
...
static int video_open(VideoState *is, int force_set_video_mode)
{
    ...
    if (screen && is->width == screen->w && screen->w == w
       && is->height== screen->h && screen->h == h && !force_set_video_mode)
        return 0;
	//by liqingyu 2013.04.08 start 
	if( FFQLAY_parentWidth == 0 )
	    screen = SDL_SetVideoMode(w, h, 0, flags);
	else
	    screen = SDL_SetVideoMode(FFQLAY_parentWidth, FFQLAY_parentHeight, 0, flags);
	//screen = SDL_SetVideoMode(w, h, 0, flags);
	//by end
    if (!screen) {
    ...
    is->height = screen->h;
	//by liqingyu 2013.04.08 start 
	is->is_file_end_reached = 0;
	is->is_video_end_reached = 0;
	is->is_audio_end_reached = 0;
	is->is_subtitle_end_reached = 0;
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, const char *vfilters, AVFrame *frame)
{
	...
	//by liqingyu start
	{
		fprintf(stderr,"ret < 0 goto fail; \n");
        goto fail;
	}//by end
    ...
    //by liqingyu start
	{
		fprintf(stderr,"2 goto fail; \n");
        goto fail;
	}//by end
    if ((ret = avfilter_link(filt_crop, 0, filt_out, 0)) < 0)
        //by liqingyu start
	{
		fprintf(stderr,"3 goto fail; \n");
        goto fail;
	}//by end
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        //by liqingyu 2013.04.12 start
        {
            is->is_video_end_reached = 1;
            fprintf(stderr,"is_video_end_reached..\n");
            goto the_end;
        }
		//by end
        if (!ret)
        ...
        if (   last_w != frame->width
            ...
            if ((ret = configure_video_filters(graph, is, vfilters, frame)) < 0) {
				fprintf(stderr,"configure_video_filters fail!!!!\n");
                SDL_Event event;
		...
		//by liqingyu start
		{
			fprintf(stderr,"ret < 0 to the end\n");
			goto the_end;
		}
        //by end   
    }
...
static int audio_decode_frame(VideoState *is, double *pts_ptr)
{
    ...
    for (;;) {
        ...
        {
            if( is->is_audio_end_reached == 0 )
            {
				is->is_audio_end_reached = 1;
                fprintf(stderr,"audio_end_reached..\n");
            }
            return -1;
        }
        //by end
...
//      of the seek_pos/seek_rel variables
        ...
        if (eof) {
			...
			#ifdef FFPLAYDEBUG
			//test decode video and audio size
			//fprintf(stderr,"audioq.size:%d\n",is->audioq.size);
            //fprintf(stderr,"videoq.size:%d\n",is->videoq.size);
			#endif
            //by end
        ...
        if (ret < 0) {
            //by liqingyu 2013.04.12 start
			//here we wait audio and video complete..
            if( is->is_file_end_reached )
			{
				if( is->video_st != NULL && is->is_video_end_reached )
					break;
				if( is->video_st == NULL && is->is_audio_end_reached )
					break;
			}
            //by end
	...
	//by liqingyu 2013.04.12 start
    //while (!is->abort_request) {
    while (!is->abort_request && ((is->video_st != NULL && !is->is_video_end_reached) || (is->video_st == NULL && !is->is_audio_end_reached) ) ) {
	//by end
 ...
 fail:
	...
	{	
		fprintf(stderr,"FF_COMPLETE_EVENT ->\n");
        SDL_Event event;
        event.type = FF_COMPLETE_EVENT;
        event.user.data1 = is;
        SDL_PushEvent(&event);
	}
    //by end
...
// the signature is modified by Liqingyu
...
//by liqingyu 2013.04.12 remove the warnning start
...
//by liqingyu 2013.04.01 end

{
    int flags;
    //by liqingyu 2013.04.11 clean the warning start
    //VideoState *is;  
    char dummy_videodriver[] = "SDL_VIDEODRIVER=dummy";
	//char * sdl_env;
    
    // by liqingyu 2013.04.02 start 
    //av_log_set_flags(AV_LOG_SKIP_REPEATED);
    //parse_loglevel(argc, argv, options);
	FFQLAY_hwndParent = hwndParent;
	FFQLAY_parentWidth = width;
	FFQLAY_parentHeight = height;
    //by end	
	...
	if( is_SDL_init == 0 )
	{
		if (SDL_Init (flags)) {
			...
			//FFQLAY_error = FFQLAY_ERROR_SDL;
			//snprintf(FFQLAY_error_str, 512, "Could not initialize SDL - %s\n", SDL_GetError());
			return -1;
		}
		is_SDL_init = 1;
	}
	...
	if(hwndParent != NULL)
    {
		screen = SDL_SetVideoMode(width, height, 24, SDL_NOFRAME| SDL_HWSURFACE|SDL_ASYNCBLIT|SDL_HWACCEL|SDL_RESIZABLE);
		if( screen != NULL )
		{
			...
			if( SDL_GetWMInfo(&wmInfo) != -1 )
			{
				...
				SetParent(hwndSDL, hwndParent);
				MoveWindow(hwndSDL, 0, 0, width, height, FALSE);
			}
		}   
    }
    //by end
    ...
    //by liqingyu 2013.04.02 start 
	if( FFQLAY_video_state_mutex == NULL )
		FFQLAY_video_state_mutex = SDL_CreateMutex();
...
{
	SDL_LockMutex(FFQLAY_video_state_mutex);
	if (cur_stream)
	{
        toggle_pause(cur_stream);
		SDL_UnlockMutex(FFQLAY_video_state_mutex);
		return 0;
	}
	SDL_UnlockMutex(FFQLAY_video_state_mutex);
	return -1;
...
__declspec (dllexport) double FFQLAY_get_position(void)
{
    ...
    if (cur_stream)
	{
		...
		double duration = cur_stream->ic->duration / AV_TIME_BASE;
		//
		if( cur_stream->ic->start_time != AV_NOPTS_VALUE )
			start_time = cur_stream->ic->start_time / AV_TIME_BASE;
...
__declspec (dllexport) int FFQLAY_set_position(double position)
{
    ...
    if (cur_stream)
	{
		double incr = 1.0;
		double start_time = 0.0;
		double cur_pos = get_master_clock(cur_stream);
        ...
        */
		if( cur_stream->ic->start_time != AV_NOPTS_VALUE )
			start_time = cur_stream->ic->start_time / AV_TIME_BASE;
		incr = position - cur_pos - start_time;
        //fprintf(stderr,"seek_flags:%d\n",cur_stream->seek_flags);
		//stream_seek(cur_stream, (int64_t)(position * AV_TIME_BASE), incr,cur_stream->seek_flags);
		stream_seek(cur_stream, (int64_t)(position * AV_TIME_BASE), incr,0);
        SDL_UnlockMutex(FFQLAY_video_state_mutex);
    }
	SDL_UnlockMutex(FFQLAY_video_state_mutex);
	return -1;
...
{
	int ret = FFQLAY_PLAY_STATE_STOPPED;
    SDL_LockMutex(FFQLAY_video_state_mutex);
	if( cur_stream )
	{
		if( cur_stream->paused )
			ret = FFQLAY_PLAY_STATE_PAUSED;
		else if( cur_stream->abort_request )
			ret = FFQLAY_PLAY_STATE_ABORT;
		else
			ret = FFQLAY_PLAY_STATE_PLAYING;
	}
	SDL_UnlockMutex(FFQLAY_video_state_mutex);
	return ret;
}
